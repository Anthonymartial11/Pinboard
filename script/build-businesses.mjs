...
// Plurals matter. "Owners, Oppenheimer Companies" and "Founders/partners,
// Sullivan & Reberger" are both ownership lines written about two people, and
// a word boundary after the singular silently skipped every one of them.
const ROLE_OWN = /\b(sole owners?|co-owners?|owners?|proprietors?|principal owners?|managing owners?)\b/i;
const ROLE_FOUND = /\b(co-founders?|founders?|founded|founding partners?)\b/i;
...
function entitiesFromRole(rec) {
  ...
  for (const clause of role.split(/;|\u2014| \u2013 /)) {
    ...
    if (!ent) {
      const of = c.match(/\b(?:co-)?(?:founders?|owners?|proprietors?)\s+of\s+(?:the\s+)?(.+)$/i);
      if (of) ent = of[1];
    }
    // "IFF founder"
    if (!ent) {
      const before = c.match(/^(.+?)\s+(?:co-)?(?:founders?|owners?|proprietors?)\b/i);
...
// Control and family holdings are their own relations. A man who runs his
// family's ranch does not personally own it, and a family office someone
// controls is not a company they own outright. Collapsing either into "owns"
// would be the same mistake as calling a school principal a business owner.
const RANK = { owns: 0, "co-owns": 1, controls: 2, "co-controls": 3, "family-owned": 4, founded: 5, "co-founded": 6, leads: 7 };
...
for (const rec of NODES) {
  ...
  for (const f of entitiesFromRole(rec)) {
    if (f.isOwn) add(rec.id, /co-owners?/i.test(f.clause) ? "co-owns" : "owns", f.ent, "role", rec.role);
...
// Researched entries, merged last so they sit alongside what the records said
// and are marked as researched with the outlet they came from.
const RESEARCH = JSON.parse(fs.readFileSync(new URL("../research-businesses.json", import.meta.url).pathname, "utf8"));
let researched = 0;
for (const [id, list] of Object.entries(RESEARCH)) {
  if (id.startsWith("_")) continue;
  for (const r of list) {
    const arr = (OUT[id] = OUT[id] || []);
    const key = r.name.toLowerCase().replace(/[^a-z0-9 ]/g, "").replace(/\s+/g, " ").trim();
    let e = arr.find((x) => (x.name || "").toLowerCase().replace(/[^a-z0-9 ]/g, "").replace(/\s+/g, " ").trim() === key);
    if (!e) { e = { name: r.name, rels: [], source: "researched", quote: r.quote }; arr.push(e); }
    for (const rel of r.rels) if (!e.rels.includes(rel)) e.rels.push(rel);
    e.rels.sort((a, b) => (RANK[a] ?? 99) - (RANK[b] ?? 99));
    e.cite = r.cite; e.researched = true;
    if (r.flag) e.flag = r.flag;
    if (r.quote) e.quote = r.quote;
    researched++;
  }
}
console.log("researched entries merged:", researched);
